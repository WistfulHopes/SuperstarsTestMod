...
[HarmonyPatch(typeof(SysDialogManager), nameof(SysDialogManager.CreateCharacterSelectWindow))]
class CreateCSelectPatch
{
    static bool Prefix(SysDialogManager __instance, bool isChara5, Il2CppSystem.Action<int, UICharacterSelectWindow.TransferSelectdAllCharaData> actionSelectPlayerAction, 
        UICharacterSelectWindow.TransferSelectdAllCharaData defaultData, ref GameObject __result)
    {
        var handle = Addressables.LoadAssetAsync<GameObject>("Character4SelectWindow");
        var CSelect = handle.WaitForCompletion();
        __instance.character4SelectWindowPrefab = CSelect.GetComponent<UICharacterSelectWindow>();
        
        return true;
    }
}

[HarmonyPatch(typeof(UICharacterSelectWindow.CharaSelectData), nameof(UICharacterSelectWindow.CharaSelectData.DecidePlayer))]
class DecidePlayerPatch
{
    static bool Prefix(UICharacterSelectWindow.CharaSelectData __instance, int playerID)
    {
        __instance.animator.SetTriggerID(UIDefine.On_decide);
        __instance.playerPointerList[playerID].gameObject.SetActive(false);
        string str = String.Format(null, "P{0}", playerID + 1);
        var localizeString = UIDefine.GetLocalizeString(SysLocalizationManager.EStringTable.main_menu, str);
        __instance.decidePointer.SetText(localizeString);
        var db = DataBaseSingleton<DB_UI>.Get;

        var charaColorIndex = __instance.playerType > PlayerBase.EPlayerType.Max
            ? PlayerBase.EPlayerType.Sonic
            : __instance.playerType;
        __instance.colorPlateImage.color = db.selectCharaColorList[(int)charaColorIndex];

        if (__instance.selectCharaIDList.Count >= 2)
        {
            __instance.arrowObject.gameObject.SetActive(true);
        }
...
[HarmonyPatch(typeof(GameSceneControllerBase), nameof(GameSceneControllerBase.LoadPlayerAssetData))]
class LoadPlayerPatch
{
    static void Prefix(GameSceneControllerBase __instance)
    {
        __instance.playerList.Resize<string>(101);
        __instance.playerList[(Index)100] = "Ply_Mod_M00.prefab";
    }
}